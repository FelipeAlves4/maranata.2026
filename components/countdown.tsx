...
const labels = { days: 'DIAS', hours: 'HORAS', minutes: 'MIN', seconds: 'SEG' };
...
export default function Countdown() {
  ...
  return <div className="countdown" aria-label="Contagem regressiva para 31 de outubro de 2026">{Object.entries(timeLeft).map(([unit, value]) => <div key={unit}><strong>{String(value).padStart(2, '0')}</strong><span>{labels[unit as keyof typeof labels]}</span></div>)}</div>;
}
